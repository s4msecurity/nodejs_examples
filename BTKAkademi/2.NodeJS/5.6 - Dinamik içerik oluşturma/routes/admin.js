...
router.use("/add", function(req,res,next){ //url adresinde "/add" kısmı eklendiğinde bu adrese girişi yapıldığında sistem sadece bu kısma getirmektedir
    ...
    res.render("add", {title: "Admin Add Stock"}) //bu sistemde otomatik olarak add.pug dosyası varsa bulur ve gönderir.
})
/*
...
*/
router.post("admin/add", function(req,res, next){ //gelen stoğu yazdıracağımız ekran. Bu kısımda gelen verinin post metoduyla gelmesini filtreleyerek "use" kullanmadan sadece post gelmesi durumunda çalışması sağlndı.
//database kayıt işlemleri için yapılabilir.
console.log(req.body) //body içinde formadan gelen bilgileri alınmayı sağlayabiliriz. Express ile body bilgisi eklendi.
res.redirect("/")
})
